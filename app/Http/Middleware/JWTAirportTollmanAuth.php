...
class JWTAirportTollmanAuth
{
    ...
    public function handle($request, Closure $next)
    {
        try{
            ...
            $toolman = json_decode(JWTAuth::getPayload(JWTAuth::getToken()));
        ...
        $tokenIsValid = AirportTollman::where('id', $toolman->airport_tollman->id)->where('api_token', $request->header('Authorization'))->first();
        if($tokenIsValid == null){
            return response()->json(['status'=>'error', 'data'=>'', 'message'=>'previous_token'], 400);
        }
        $request->airport_tollman = $tokenIsValid;
        return $next($request);
    }
}
