...
use App\Model\OfflineCounterman;
use Closure;
use JWTAuth;
use Tymon\JWTAuth\Exceptions\JWTException;

class JWTOfflineCountermanAPIAuth
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        try{
            ...
            $entity = json_decode(JWTAuth::getPayload(JWTAuth::getToken()));
        ...
        $tokenIsValid = OfflineCounterman::where('id', $entity->offline_counterman->id)->where('api_token', $request->header('Authorization'))->first();
        if($tokenIsValid == null){
            return response()->json(['status'=>'error', 'data'=>'', 'message'=>'previous_token'], 400);
        }
        $request->offline_counterman = $tokenIsValid;
        return $next($request);
    }
}
