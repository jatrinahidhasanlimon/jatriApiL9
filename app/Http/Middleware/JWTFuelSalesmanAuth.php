...
use App\FuelSalesman;
use Closure;
use JWTAuth;
use Tymon\JWTAuth\Exceptions\JWTException;

class JWTFuelSalesmanAuth
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        try{
            ...
            $salesman = json_decode(JWTAuth::getPayload(JWTAuth::getToken()));
        ...
        $tokenIsValid = FuelSalesman::where('id', $salesman->fuel_salesman->id)->where('api_token', $request->header('Authorization'))->first();
        if($tokenIsValid == null){
            return response()->json(['status'=>'error', 'data'=>'', 'message'=>'previous_token'], 400);
        }
        $request->fuel_salesman = $salesman->fuel_salesman;
        return $next($request);
    }
}
