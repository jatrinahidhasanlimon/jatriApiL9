...
use App\Model\WaybillChecker;
use Closure;
use JWTAuth;
use Tymon\JWTAuth\Exceptions\JWTException;

class JWTWaybillCheckerAuth
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        try{
            ...
            if($user->env == config('app.env') && $user->type == 'waybill_checker') {
                $tokenIsValid = WaybillChecker::find($user->sub);
                if ($tokenIsValid == null || $tokenIsValid->api_token != $request->header('Authorization')) {
                    return response()->json(['status' => 'error', 'data' => '', 'message' => 'previous_token'], 400);
                }
                $request->waybill_checker = $tokenIsValid;
                return $next($request);
            }
